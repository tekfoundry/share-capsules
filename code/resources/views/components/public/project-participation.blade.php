@php
    ...
    $githubUrl = $repositoryUrl !== '' ? $repositoryUrl : 'https://github.com/tekfoundry';
@endphp
...
<section id="participate" class="border-t border-line bg-canvas py-16 sm:py-20">
    <div class="mx-auto grid max-w-7xl gap-10 px-5 sm:px-8 lg:grid-cols-[0.72fr_1.28fr] lg:items-center lg:px-10">
        <div>
            <a class="inline-block rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand" href="https://tekfoundry.com" rel="noreferrer">
        ...
        <div>
            ...
            <div class="mt-7 flex flex-col gap-3 sm:flex-row">
                ...
                <a class="inline-flex min-h-11 items-center justify-center rounded-xl border border-line bg-white px-5 text-sm font-bold text-ink transition hover:border-brand/30 hover:text-brand focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand" href="{{ $githubUrl }}" rel="noreferrer">
                    {{ $repositoryUrl !== '' ? 'Review the project on GitHub' : 'Follow TekFoundry on GitHub' }}
                </a>
            </div>

            @if ($repositoryUrl === '')
                <p class="mt-4 text-xs leading-5 text-muted">The direct public project repository link will appear when publication is configured.</p>
            @endif
        </div>
    </div>
</section>
