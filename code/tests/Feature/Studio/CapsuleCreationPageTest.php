...
final class CapsuleCreationPageTest extends TestCase
{
    ...
    public function test_a_verified_creator_can_prepare_metadata_and_the_v1_policy(): void
    {
        ...
        $this->actingAs($creator)
            ...
            ->assertSee('Time Capsule, Limit Capsule, and Trust Capsule policies')
            ->assertSee('these policies can be combined')
            ...
            ->assertSee('Trust Capsule policy: require a viewer trust check')
            ->assertSee('Use a viewer trust score to help decide whether protected content can open')
            ->assertSee('recent usage patterns and quick human challenges')
            ->assertSee('does not verify identity, unique personhood, benign intent, or one-human-one-account')
            ->assertSee('Current access summary')
            ->assertSee('This summary updates as you configure the policies')
            ->assertSee('Always required')
            ->assertSee('Time policy')
            ->assertSee('No opening dates configured')
            ->assertSee('Limit policy')
            ->assertSee('No opening limits configured')
            ->assertSee('Trust policy')
            ->assertSee('No viewer trust check configured')
