...
export interface CreatorSigningKeyRecord {
    ...
    readonly status: 'active' | 'retiring' | 'revoked' | 'expired';
    readonly createdAt: string;
    readonly statusChangedAt: string;
    readonly recoveryStatus: 'required' | 'confirmed';
    readonly recoveryConfirmedAt?: string;
}

export interface CreatorCapsuleBuildInput<TToken = unknown, TDevice = unknown> {
...
export interface BrokerKeyRegistrar<TToken = unknown, TDevice = unknown> {
    register(
...
export class CreatorCapsuleBuilderV1<TToken = unknown, TDevice = unknown> {
    public constructor(
        private readonly configuration: CreatorCapsuleBuilderConfiguration,
        private readonly broker: BrokerKeyRegistrar<TToken, TDevice>,
    ...
    public async build(input: CreatorCapsuleBuildInput<TToken, TDevice>): Promise<BuiltCapsuleV1> {
        if (
