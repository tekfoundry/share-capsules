...
The extension iframe owns the accessible state announcements for protected content. Loading and opening work sets the frame region busy; consent, connection, denial, revocation, limit, unsupported-profile, network, temporary-service, rate-limit, and security-failure blockers clear the busy state and announce reviewed locked messages with any safe retry action. Opened content hides routine protocol chrome visually but keeps the successful status available to assistive technology.
...
Viewer failures are mapped to reviewed, user-safe categories rather than raw protocol detail. Broker CTX errors such as invalid or stale tickets, invalid device proof, unavailable release, opening limits, policy mismatch, automation protection, temporary service failures, and HTTP rate limiting produce actionable messages while keeping tickets, proofs, keys, identifiers, and broker internals out of the Host page. Temporary fetch, authorization, broker availability, and rate-limit failures keep the extension frame visible with a retry action that starts a fresh opening attempt rather than replaying a stale ticket. Broker verification dependencies, including CTX provider signing-key lookup, must fail as retryable temporary availability problems rather than as viewer-blaming ticket failures.
...
Viewer extension storage is allow-listed for durable Viewer connection state and standing disclosure consent only. It must not accept plaintext, content keys, one-use authorization tickets, key-release proofs, decrypted object URLs, or other protected opening material. Device private keys remain in extension-owned IndexedDB as non-exportable browser `CryptoKey` objects.

Multiple same-page Viewer frames may observe the same account connection and continue automatically, but the extension serializes the online authorization, broker release, and decrypt/render opening pipeline. This avoids bursty release attempts, makes local development deterministic, and creates a clear boundary for bulk-opening safety without making normal pages feel unlike ordinary web media.
...
Site permission and automatic-opening consent are separate. After a viewer explicitly allows automatic opening for a Host site, eligible Capsules should behave like ordinary protected web media: if the extension has a usable Viewer connection and consent still applies, they may authorize and render automatically using the existing CTX account connection and standing disclosure consent.
...
Automatic opening is revocable per site, re-prompts when policy or disclosure meaning changes, applies queueing and safety limits to unusual bulk pages, and explains that committed releases count against Capsule limits. Visibility alone is not a hard blocker because ordinary sites often use tabs, accordions, carousels, modals, and script-controlled show/hide behavior. Bulk protection is therefore based on rate, count, lifecycle, and explicit viewer consent rather than treating every hidden element as malicious. Without standing consent, frames remain locked until the viewer connects and approves the required disclosure.
