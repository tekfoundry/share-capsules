...
The deletion worker uses bounded batches and independently locked transactions. It rechecks the deadline after acquiring the lock, erases every configured account-linked data repository, and deletes the account only if every required participant succeeds. A failure leaves the closed account unavailable and eligible for retry rather than producing a partially active account. A replacement registration using the same email receives a new identifier with no inherited account state.

Ordinary deletion leaves no account-level tombstone. If an active sanction exists, V1 retains only a keyed HMAC of the normalized verified email, sanction category, imposed and expiry timestamps, and an appeal reference until sanction reversal, sanction expiry, or 90 days from deletion, whichever comes first. The implementation uses a dedicated deployment-specific 32-byte HMAC-SHA-256 key, an immediate reversal path, and hourly expiry pruning. It is restricted to same-email signup enforcement and authorized appeal or security operations, contains no behavioral history or device fingerprint, and cannot link a replacement account to the deleted account. This is limited account-abuse friction, not personhood or duplicate-person detection.

Deleted data may persist in encrypted disaster-recovery backups for no more than 30 additional days. Those backups are unavailable to ordinary product, analytics, or security-query paths. Before a restored backup serves traffic, the system reapplies deletion obligations from an independently stored ledger containing only an opaque entry ID, internal account ID, and deletion/retention timing. A fresh restore UUID and fail-closed readiness gate prevent traffic until replay completes. Ledger entries are pruned with the 30-day backup lifetime and never preserve email, credentials, trust profile, activity history, device data, or Capsule data.
