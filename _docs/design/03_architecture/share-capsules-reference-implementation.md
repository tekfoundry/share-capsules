...
The extension is a public OAuth client and has no embedded client secret. It receives short-lived, narrowly scoped access tokens without receiving or retaining the account password. Tokens are sender-constrained to the registered Ed25519 Viewer proof key, and API requests prove possession of that key. The separate X25519 Viewer agreement key receives HPKE-wrapped content keys. If refresh tokens are issued, they are rotated, revocable, and treated as sensitive extension state. Device Authorization is deferred as a possible fallback for future platforms.

Laravel Passport implements the authorization server and PKCE validation. The fixed V1 client permits only Authorization Code, has no secret, registers exactly one environment-specific extension callback, requires verified-account consent, and currently issues only a ten-minute `extension:connect` bearer token. No CTX protected-resource route accepts that interim bearer token. Device registration and DPoP binding must be completed before it becomes a CTX credential; refresh-token issuance remains disabled until rotation and replay handling are implemented.
