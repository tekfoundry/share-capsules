...
- 🟨 Configure TLS for both hostnames, security headers, secret management, broker key-custody settings appropriate to the approved prototype posture, backups, deletion-ledger restoration, and alerting. TLS, prototype key custody, DigitalOcean daily database-server backups, and external uptime monitoring for both health endpoints are configured; restore drill plus queue/scheduler/backup/security alerting evidence remain. Runbook: [§7](../operations/phase12-production-runbook.md#7-tls-headers-secrets-backups-and-alerts).
...
- 🟨 Audit the complete public working tree and history for secrets, local data, logs, generated artifacts, unrelated private information, and third-party licensing obligations. Dependency audits are clean, the release supply-chain check is green, full local CI and browser checks pass, current-source high-confidence secret scans are clean, and the unpacked production extension directory is excluded from source control; GitHub secret-scanning/platform evidence and final dependency-license review remain. Runbook: [§11](../operations/phase12-production-runbook.md#11-public-repository-and-history-audit).
- 🟨 Configure the public GitHub repository, Discussions, protected primary branch, required CI, and private vulnerability-reporting path. Repo-side security automation and controls are documented from Phase 10; final platform verification remains. Runbook: [§11](../operations/phase12-production-runbook.md#11-public-repository-and-history-audit) and [public repository security controls](../operations/public-repository-security-controls.md).
- 🟨 Publish the static reference Host and downloadable example Capsules. A same-application public Phase 12 test page hosts a production-created TekFoundry logo Capsule at `/phase12/capsule-test`; production verified the revisioned Capsule URL `/phase12/capsules/tekfoundry-logo-r1.capsule` and current source moves that URL outside session middleware; deployment of the no-cookie hardening and the separate static reference Host remain. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
- 🟨 Verify the production static-host contract: public noncredentialed CORS, immutable revision URLs, bounded content lengths, accepted Capsule media type, and cache/header behavior that works from a separately hosted page. Same-origin public hosting, CORS/cache/media headers, and extension rendering are verified; current source also tests that the same-app Capsule route emits no cookies, while no-cookie production verification and separate-origin CORS/static-host review remain. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
