...
- 🟨 Build the extension without remotely hosted code, publish source/build hashes, and submit the fixed production identity to the Chrome Web Store. Production package was built, verified, hashed, uploaded to the reserved item, and used for the production acceptance check; final Chrome Web Store review submission/publication remains deferred. Runbook: [§9](../operations/phase12-production-runbook.md#9-extension-build-and-store-submission).
- 🟨 Publish creator, viewer, privacy, security-limit, account-deletion, and compatible-Host documentation. Documentation is deployed with the application, but final review of public URLs and release evidence remains. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
- 🟨 Publish the project README, Apache License 2.0 notice, sponsorship/contact statement, contribution guide, governance guidance, code of conduct, security policy, and issue/pull-request templates. Core repository materials exist; final public repository settings/template review remains. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
- 🟨 Audit the complete public working tree and history for secrets, local data, logs, generated artifacts, unrelated private information, and third-party licensing obligations. Dependency audits are clean, CI is green, and the unpacked production extension directory is excluded from source control; full secret/history/licensing review remains. Runbook: [§11](../operations/phase12-production-runbook.md#11-public-repository-and-history-audit).
...
- 🟨 Publish the static reference Host and downloadable example Capsules. A same-application public Phase 12 test page now hosts a production-created TekFoundry logo Capsule at `/phase12/capsule-test`, and the encrypted Capsule is available at `/capsules/phase12/tekfoundry-logo.capsule`; the separate static reference Host remains. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
- 🟨 Verify the production static-host contract: public noncredentialed CORS, immutable revision URLs, bounded content lengths, accepted Capsule media type, and cache/header behavior that works from a separately hosted page. Same-origin public hosting and extension rendering are verified; separate-origin CORS and cache/header review remain. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
...
- 🟨 Run clean-account creator and viewer acceptance tests against production-like infrastructure. Production OAuth, device registration, Capsule creation, broker registration, public hosting, CTX authorization/redemption, and local extension rendering were verified with the TekFoundry logo Capsule; revocation, account/device revocation, and cleanup checks remain. Runbook: [§14](../operations/phase12-production-runbook.md#14-acceptance-load-and-concurrency-testing).
