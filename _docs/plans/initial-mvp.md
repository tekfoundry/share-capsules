...
- ✅ Finalize web-like automatic opening: after extension connection and applicable consent, eligible Capsules attempt to open automatically while blockers such as no extension, missing connection, missing consent, denial, revocation, limits, unsupported content, temporary service failures, and rate limits are explained clearly.
- ✅ Prevent unusual bulk pages from silently consuming releases with queueing, retry-aware rate handling, lifecycle checks, and page/session safety limits while preserving normal Host show/hide layouts such as tabs, accordions, carousels, and modals.
...
- ✅ Require fresh online authorization on reload or reopen; persist no plaintext or content key in ordinary storage or cache.
- ✅ Implement accessible loading, consent, denial, revocation, device-limit, unsupported-profile, network, and security-failure states.
- ✅ Implement the no-extension install/onboarding link with safe return state and no credentials or tokens in URLs.
...
- The full non-browser gate passes with 378 TypeScript tests and 273 PHP tests / 1,339 assertions. `npm run build` now also produces the loadable extension bundle. `_docs/operations/phase7-creator-runtime-manual-test.md` records successful local service preparation, unpacked extension loading, first Capsule and recovery-kit creation, restored signing identity, matching creator signing-key identity, and negative recovery-code behavior in the real browser runtime.
...
- The public no-extension path now has a dedicated `/viewer/install` onboarding page and safe-return parser. Host fallback examples link there with only ordinary public navigation state; tests reject credentials, fragments, token/code/ticket/proof/key/recovery query names, and non-web schemes.
...
- Discovery now treats an explicit `<capsule-viewer>` as the contract even when Host layout currently hides it. This preserves ordinary web behavior for tabs, accordions, carousels, modals, and script-controlled reveal patterns. Bulk release protection is handled by scoped consent, queueing, rate handling, lifecycle checks, and page/session safety limits rather than by using visibility as a universal blocker.
- Temporary fetch, CTX authorization, broker availability, and rate-limit failures now keep the extension frame available with a retry action. Retrying starts a fresh opening attempt and therefore obtains fresh authorization material instead of replaying a stale ticket. Permanent denials, policy failures, unsupported content, and exhausted limits still fail closed with safe locked messaging.
- Viewer extension storage is now guarded by an allow-list that accepts only Viewer connection state and standing disclosure consent records. Plaintext, unwrapped content keys, authorization tickets, release proofs, object URLs, and decrypted payload material are rejected as durable storage keys; fetch, authorization, broker release, and payload-renderer tests continue to lock no-store network behavior, content-key/plaintext zeroing, and object URL disposal.
- The Viewer iframe now exposes explicit loading, action-required, error, and opened states through `aria-busy`, a stable labelled region, and polite status announcements. Opened content still hides protocol chrome visually while preserving accessible status text. Focused tests cover the frame-state mapping and the reviewed safe messages for network, account, device, policy, revocation, opening-limit, automation-protection, temporary-service, and rate-limit blockers.
...
Each Phase 12 task includes either a direct runbook reference or a reference to the combined [Phase 12 Production Deployment Runbook](../operations/phase12-production-runbook.md).

- ✅️ Confirm that the release candidate provides a coherent user-visible capability and explicitly approve production deployment; phase completion alone does not trigger a deployment. Runbook: [Phase 12 Production Deployment Runbook §1](../operations/phase12-production-runbook.md#1-release-approval).
- ✅️ Reserve the production Chrome Web Store item and fixed production extension ID before finalizing Forge environment values or OAuth redirect configuration. Recorded in [Chrome Web Store Production Identity](../operations/chrome-web-store-production-identity.md). Runbook: [§2](../operations/phase12-production-runbook.md#2-reserve-production-extension-identity).
- 🟨 Reconcile the existing Forge production environment against `code/.env.production.example`, including the accepted same-install broker posture, and record the sanitized baseline in the production change ledger. Core production identities, broker DB, OAuth, Passport keys, CTX signing keys, TLS, queue, scheduler, accepted prototype key-custody settings, daily all-database backups with seven-day retention, code backup through Git, and `.env` backup through 1Password are recorded; the manual restore process is accepted without a Share Capsules-specific drill for MVP, while alerting detail and final baseline review remain. Runbook: [§3](../operations/phase12-production-runbook.md#3-environment-reconciliation).
- ✅️ Deploy one Laravel application on Forge with MySQL, Redis, queues, scheduler, and both HTTPS hostnames: `sharecapsules.com` for the control plane and `broker.sharecapsules.com` for the Key Broker origin. App, database, Redis, hostnames, migrations, smoke checks, every-minute scheduler execution, and a PHP 8.4 Redis queue worker are live. Runbook: [§4](../operations/phase12-production-runbook.md#4-forge-site-and-dns), [§5](../operations/phase12-production-runbook.md#5-forge-deployment-script), and [§6](../operations/phase12-production-runbook.md#6-queue-and-scheduler).
- ✅️ Configure `broker.sharecapsules.com` to resolve to the same Forge site for the prototype, keep `SHARECAPSULES_COMPONENT=control-plane`, and verify broker routes are host-bound to the broker origin while app routes remain host-bound to the app origin. Runbook: [§4](../operations/phase12-production-runbook.md#4-forge-site-and-dns) and [§13](../operations/phase12-production-runbook.md#13-production-smoke-checks).
- ✅️ Record the reduced-isolation broker tradeoff for this release and preserve the hardening path where `broker.sharecapsules.com` can later move to a broker-only Forge site using `code/.env.broker.production.example`. Includes accepted same-install local key-custody posture for this MVP prototype. Runbook: [overview](../operations/phase12-production-runbook.md#phase-12-production-deployment-runbook) and [§16](../operations/phase12-production-runbook.md#16-final-release-evidence).
- 🟨 Record every production environment-variable name, migration, infrastructure or identity change, deployment action, verification result, and rollback consideration without committing secret values. Deployment, OAuth, signing-key, broker DB, TLS, key-custody, and smoke-check evidence are recorded; final release evidence and deployed-change promotion remain. Runbook: [§3](../operations/phase12-production-runbook.md#3-environment-reconciliation), [§5](../operations/phase12-production-runbook.md#5-forge-deployment-script), and [§16](../operations/phase12-production-runbook.md#16-final-release-evidence).
- ✅️ Configure TLS for both hostnames, security headers, secret management, broker key-custody settings appropriate to the approved prototype posture, backups, deletion-ledger restoration, and alerting. TLS, prototype key custody, DigitalOcean daily all-database backups with seven-day retention and accepted up-to-24-hour data-loss risk, code backup through Git, `.env` backup through 1Password, a manual restore path accepted without a Share Capsules-specific drill for MVP, working UptimeRobot alerts for both health endpoints, and Forge scheduler/queue worker visibility are configured; richer internal queue/scheduler/backup/security alerting is deferred as a follow-up. Runbook: [§7](../operations/phase12-production-runbook.md#7-tls-headers-secrets-backups-and-alerts).
- ✅️ Publish stable provider and broker discovery metadata and controlled signing-key rotation procedures. Production discovery and JWKS publish the activated CTX signing key `01kyjz8e0knc0xjv7pg323ztv9`; rotation procedures remain in the operations docs. Runbook: [§8](../operations/phase12-production-runbook.md#8-discovery-and-signing-keys).
- ✅️ Build the extension without remotely hosted code, publish source/build hashes, and submit the fixed production identity to the Chrome Web Store. Production package was built, verified, hashed, uploaded to the reserved item, used for the production acceptance check, submitted for Chrome Web Store review with automatic publishing enabled, approved, and published publicly as extension `jkejpdcobbbeichpodpeoiilnalepdph`. Runbook: [§9](../operations/phase12-production-runbook.md#9-extension-build-and-store-submission).
- 🟨 Publish creator, viewer, privacy, security-limit, account-deletion, and compatible-Host documentation. Documentation is deployed with the application, source routes cover overview, instructions, technical/security-limit language, viewer install, terms, and privacy, and local CI plus browser checks pass; final production URL review after the next deploy remains. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
- ✅️ Publish the project README, Apache License 2.0 notice, sponsorship/contact statement, contribution guide, governance guidance, code of conduct, security policy, and issue/pull-request templates. The repository includes README, LICENSE, NOTICE, sponsorship/contact language, CONTRIBUTING, GOVERNANCE, CODE_OF_CONDUCT, SECURITY, funding metadata, issue templates, and a pull request template. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
- 🟨 Audit the complete public working tree and history for secrets, local data, logs, generated artifacts, unrelated private information, and third-party licensing obligations. Dependency audits are clean, the release supply-chain check is green, full local CI and browser checks pass, current-source high-confidence secret scans are clean, the unpacked production extension directory is excluded from source control, repo-side control files are locally verified, and GitHub security/platform settings are maintainer-confirmed; final dependency-license review remains. Runbook: [§11](../operations/phase12-production-runbook.md#11-public-repository-and-history-audit).
- ✅️ Configure the public GitHub repository, Discussions, protected primary branch, required CI, and private vulnerability-reporting path. Private vulnerability reporting, secret scanning, push protection, dependency graph, Dependabot, code scanning, repo-side control files, and access review are confirmed; primary branch is `master`; CodeQL targets JavaScript/TypeScript and PHP on pull requests, `master` pushes, and schedule; Discussions, branch protection/rulesets, and required checks are deferred as accepted MVP residual risk. Runbook: [§11](../operations/phase12-production-runbook.md#11-public-repository-and-history-audit) and [public repository security controls](../operations/public-repository-security-controls.md).
- 🟨 Publish the static reference Host and downloadable example Capsules. A same-application public test page hosts a production-created TekFoundry logo Capsule at `/capsule-test`, with the legacy `/phase12/capsule-test` route redirected for compatibility; production verified the revisioned no-cookie Capsule URL `/phase12/capsules/tekfoundry-logo-r1.capsule`; the separate static reference Host remains. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
- 🟨 Verify the production static-host contract: public noncredentialed CORS, immutable revision URLs, bounded content lengths, accepted Capsule media type, and cache/header behavior that works from a separately hosted page. Same-application public hosting, no-cookie CORS/cache/media headers, and extension rendering are verified; separate-origin CORS/static-host review remains. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
- ✅️ Run same-install broker smoke checks against production: app and broker `/up`, app and broker discovery metadata, broker-host `/login` returns `404`, app-host `/releases` returns `404`, and broker internal routes reject missing service credentials. Runbook: [§13](../operations/phase12-production-runbook.md#13-production-smoke-checks).
- 🟨 Run clean-account creator and viewer acceptance tests against production-like infrastructure. Production OAuth, device registration, Capsule creation, broker registration, public hosting, CTX authorization/redemption, store-installed extension rendering after reconnect, reversible per-Capsule pause/resume, and Viewer device suspend/reactivate behavior were verified with the TekFoundry logo Capsule; permanent Capsule revocation, account closure/deletion, and cleanup checks remain. Runbook: [§14](../operations/phase12-production-runbook.md#14-acceptance-load-and-concurrency-testing).
- ⬜️ Run load and concurrency tests for authorization, redemption, counters, and broker operations at the intended MVP scale. Runbook: [§14](../operations/phase12-production-runbook.md#14-acceptance-load-and-concurrency-testing).
- ⬜️ Exercise incident response, signing-key rotation, extension-version suspension, Capsule revocation, backup restoration, and rollback procedures. Runbook: [§15](../operations/phase12-production-runbook.md#15-incident-response-and-recovery-exercises).
- ⬜️ Record release evidence and explicitly approve the MVP against the release criteria below. Runbook: [§16](../operations/phase12-production-runbook.md#16-final-release-evidence).
