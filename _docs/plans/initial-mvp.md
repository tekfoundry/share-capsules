...
- 🟨 Publish the static reference Host and downloadable example Capsules. A same-application public Phase 12 test page hosts a production-created TekFoundry logo Capsule at `/phase12/capsule-test`; current source adds the tested revisioned Capsule URL `/phase12/capsules/tekfoundry-logo-r1.capsule`; deployment of that route and the separate static reference Host remain. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
- 🟨 Verify the production static-host contract: public noncredentialed CORS, immutable revision URLs, bounded content lengths, accepted Capsule media type, and cache/header behavior that works from a separately hosted page. Same-origin public hosting and extension rendering are verified; current source tests same-app CORS/cache/media headers for the Phase 12 Capsule route and passes full local CI/browser gates, while production deployment verification and separate-origin CORS/static-host review remain. Runbook: [§12](../operations/phase12-production-runbook.md#12-static-reference-host).
