...
- ⬜️ Reconcile the existing Forge production environment against `code/.env.production.example`, including the accepted same-install broker posture, and record the sanitized baseline in the production change ledger. Runbook: [§3](../operations/phase12-production-runbook.md#3-environment-reconciliation).
- ⬜️ Deploy one Laravel application on Forge with MySQL, Redis, queues, scheduler, and both HTTPS hostnames: `sharecapsules.com` for the control plane and `broker.sharecapsules.com` for the Key Broker origin. Runbook: [§4](../operations/phase12-production-runbook.md#4-forge-site-and-dns), [§5](../operations/phase12-production-runbook.md#5-forge-deployment-script), and [§6](../operations/phase12-production-runbook.md#6-queue-and-scheduler).
...
- ⬜️ Record the reduced-isolation broker tradeoff for this release and preserve the hardening path where `broker.sharecapsules.com` can later move to a broker-only Forge site using `code/.env.broker.production.example`. Runbook: [overview](../operations/phase12-production-runbook.md#phase-12-production-deployment-runbook) and [§16](../operations/phase12-production-runbook.md#16-final-release-evidence).
- ⬜️ Record every production environment-variable name, migration, infrastructure or identity change, deployment action, verification result, and rollback consideration without committing secret values. Runbook: [§3](../operations/phase12-production-runbook.md#3-environment-reconciliation), [§5](../operations/phase12-production-runbook.md#5-forge-deployment-script), and [§16](../operations/phase12-production-runbook.md#16-final-release-evidence).
- ⬜️ Configure TLS for both hostnames, security headers, secret management, broker key-custody settings appropriate to the approved prototype posture, backups, deletion-ledger restoration, and alerting. Runbook: [§7](../operations/phase12-production-runbook.md#7-tls-headers-secrets-backups-and-alerts).
- ⬜️ Publish stable provider and broker discovery metadata and controlled signing-key rotation procedures. Runbook: [§8](../operations/phase12-production-runbook.md#8-discovery-and-signing-keys).
- ⬜️ Build the extension without remotely hosted code, publish source/build hashes, and submit the fixed production identity to the Chrome Web Store. Runbook: [§9](../operations/phase12-production-runbook.md#9-extension-build-and-store-submission).
- ⬜️ Publish creator, viewer, privacy, security-limit, account-deletion, and compatible-Host documentation. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
- ⬜️ Publish the project README, Apache License 2.0 notice, sponsorship/contact statement, contribution guide, governance guidance, code of conduct, security policy, and issue/pull-request templates. Runbook: [§10](../operations/phase12-production-runbook.md#10-user-and-project-documentation).
